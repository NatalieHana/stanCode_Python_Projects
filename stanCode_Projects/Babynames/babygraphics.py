...
def draw_names(canvas, name_data, lookup_names):
    ...
    for i in range(len(lookup_names)):
        year_rank = name_data[lookup_names[i]]  # dict
        rank = []
        for year in YEARS:
            if str(year) in year_rank:
                rank.append(year_rank[str(year)])
            else:
                rank.append("*")
        for j in range(len(rank)):
            if j == len(rank) - 1:
                canvas.create_text(x2, y2, text=(lookup_names[i], rank[j]), anchor=tkinter.SW, fill=COLORS[(i % 4)])
            else:
                ...
                if rank[j] == "*":
                    ...
                    y1 = GRAPH_MARGIN_SIZE + (CANVAS_HEIGHT - 2 * GRAPH_MARGIN_SIZE) * (int(rank[j]) / 1000)
                if rank[j + 1] == "*":
                    y2 = CANVAS_HEIGHT - GRAPH_MARGIN_SIZE
                else:
                    y2 = GRAPH_MARGIN_SIZE + (CANVAS_HEIGHT - 2 * GRAPH_MARGIN_SIZE) * (int(rank[j + 1]) / 1000)
                canvas.create_line(x1, y1, x2, y2, width=LINE_WIDTH, fill=COLORS[(i % 4)])
                canvas.create_text(x1, y1, text=(lookup_names[i], rank[j]), anchor=tkinter.SW, fill=COLORS[(i % 4)])


# main() code is provided, feel free to read through it but DO NOT MODIFY
def main():
    # Load data
    name_data = babynames.read_files(FILENAMES)  # Python import 另一py檔
